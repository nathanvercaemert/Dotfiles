...
#+BEGIN_SRC emacs-lisp
  (use-package company
    ...
    :config
    ;; these nice hooks replace the backends list buffer-locally
    ;; the list is updated by lsp (automatically) where appropriate
    ;; (at least when I call lsp manually, may have to change if I start lsp automatically)
    (defun my-company-prog-hook () ; only dabbrev and files backends are accessible from prog mode
      (setq-local company-backends '((company-dabbrev-code company-files))))
    (add-hook 'prog-mode-hook #'my-company-prog-hook)
    (defun my-company-text-hook () ; only files backend is accessible from text mode
...
#+BEGIN_SRC emacs-lisp
  ;; custom functions to override org mode paragraph movement
  ;; note that org movement still available using C-<up>/<down>
  ...
  (bind-key* "<f5> 3" 'my-forward-paragraph)
  (bind-key* "S-<f5> #" 'forward-paragraph-with-shift-select)
  (bind-key* "<f5> 2" 'my-backward-paragraph)
